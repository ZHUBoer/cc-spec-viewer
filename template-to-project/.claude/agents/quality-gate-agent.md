...
tools: Read, Bash, Grep, LS, Glob, mcp__contract-doc__get_contract_doc, mcp__xtaro-zx-library__xtaro-getting-started, mcp__ztrip-nfes__ztrip-nfes-getting-started, mcp__xtaro-zx-library__xtaro-zx-search, mcp__ztrip-nfes__ztrip-nfes-search, mcp__xtaro-zx-library__xtaro-zx-components-info, mcp__xtaro-zx-library__xtaro-zx-apis-info
color: red
skills: ast-grep, zx-h5-develop-experience
---
...
### 组件/端能力查询（按分组能力，避免硬编码工具名）

| 分组 | 说明 | tools |
| --- | --- | --- |
| overview | 业务基建/框架全景认知能力。用于在任务开始时建立全局上下文，了解框架结构、核心规范与组件和 API 概览等开发指南。 | `mcp__xtaro-zx-library__xtaro-getting-started`, `mcp__ztrip-nfes__ztrip-nfes-getting-started` |
| search | 探索与发现能力。用于在需求模糊或未知具体实现方案时，通过自然语言检索匹配最相关的文档内容、代码示例、解决方案和最佳实践。 | `mcp__xtaro-zx-library__xtaro-zx-search`, `mcp__ztrip-nfes__ztrip-nfes-search` |
| specifications | 精准规格查阅能力。用于在已确定具体组件或 API 名称后，获取其详细的介绍、属性说明、使用方式和代码示例。 | `mcp__xtaro-zx-library__xtaro-zx-components-info`, `mcp__xtaro-zx-library__xtaro-zx-apis-info` |
...
- **overview tools**：`mcp__xtaro-zx-library__xtaro-getting-started`, `mcp__ztrip-nfes__ztrip-nfes-getting-started`
- **search tools**：`mcp__xtaro-zx-library__xtaro-zx-search`, `mcp__ztrip-nfes__ztrip-nfes-search`
- **specifications tools**：`mcp__xtaro-zx-library__xtaro-zx-components-info`, `mcp__xtaro-zx-library__xtaro-zx-apis-info`
...
- **开发过程中 agent 需要的开发经验/知识，识别合适场景，按需使用。**：`zx-h5-develop-experience` skill, 提供智行公共前端开发流程规范和经验，指导 Agent 编写高质量、标准化的 H5 业务代码。涵盖：请求、分享、埋点、异常处理、登录和渐进式渲染。适用于实现 H5 业务功能阶段。
