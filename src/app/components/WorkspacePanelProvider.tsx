...
import { SpecContextPanel } from "@/components/spec-dashboard/SpecContextPanel";
import {
  type PanelMode,
...
export const WorkspacePanelProvider: FC<WorkspacePanelProviderProps> = ({
  children,
}) => {
  // Panel State
  const [activeMode, setActiveMode] = useState<PanelMode>("none");
  ...
  // Spec State
  const [specContext, setSpecContext] = useState<unknown | null>(null);
  ...
  const closeBrowser = useCallback(() => {
    ...
    setBrowserInputUrl("");
    setBrowserReloadKey(0);
  }, []);

  const reloadBrowser = useCallback(() => {
    if (iframeRef.current) {
      iframeRef.current.src = currentBrowserUrl || browserUrl || "";
    }
  }, [currentBrowserUrl, browserUrl]);

  // Track iframe URL changes
  useEffect(() => {
    if (activeMode !== "browser" || !iframeRef.current) return;
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (iframe?.contentWindow?.location.href) {
          ...
          if (newUrl !== "about:blank" && newUrl !== currentBrowserUrl) {
  ...
  }, [activeMode, currentBrowserUrl]);

  const handleBrowserUrlSubmit = useCallback(() => {
    if (browserInputUrl) {
      if (isBlockedDomain(browserInputUrl)) {
        toast.warning("This website cannot be previewed instantly.", {
          description: "It blocks embedded views. Click to open in a new tab.",
          action: {
            label: "Open Link",
            onClick: () => window.open(browserInputUrl, "_blank"),
          },
          duration: 5000,
        });
        return;
      ...
      setCurrentBrowserUrl(browserInputUrl);
    }
  }, [browserInputUrl]);

  const handleBrowserUrlKeyDown = useCallback(
  ...
  const openSpec = useCallback((context: unknown) => {
    setActiveMode("spec");
    setSpecContext(context);
  }, []);
        ...
        >
            ...
            <div className="flex flex-col h-full min-h-0">
              <div className="flex items-center gap-1.5 px-2 py-1.5 border-b border-border bg-muted/30 flex-shrink-0">
                ...
                <input
                  ...
                  onKeyDown={handleBrowserUrlKeyDown}
          ...
          {activeMode === "spec" && (
            <div className="flex flex-col h-full min-h-0 relative">
              <div className="absolute top-2 right-2 z-10">
                ...
                >
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    role="img"
                    aria-labelledby="close-spec-icon-title"
                  >
                    <title id="close-spec-icon-title">Close</title>
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
              ...
              <SpecContextPanel context={specContext} />
            </div>
          )}
        </div>
      )}
